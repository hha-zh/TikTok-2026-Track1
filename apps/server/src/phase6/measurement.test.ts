  ...
  it("runs three policies over the scenario matrix and writes the summary", async () => {
    ...
    const summary = {
      ...
      runtimeLayers: {
        ...
        externalContainerCodexArk: "PROVEN_BY_ISOLATED_PROBE",
      },
      provenance: {
        OBSERVED: [
          "outcome",
        ...
        NOT_MEASURED_BY_DETERMINISTIC_SUITE: [
          "real model token counts (see phase6-container-probe.json for external proof)",
          "real provider latency (see phase6-container-probe.json for external proof)",
          "real container startup cost (see phase6-container-probe.json for external proof)",
        ],
      },
      authorizeOverhead: {
      ...
      authorityBudgetInteraction: {
        note:
          "Authority and Budget/Horizon are parallel constraints. hardEligible captures " +
          "hard governance and capacity; planningFit distinguishes FITS_ESTIMATE from " +
          "ESTIMATED_SHORTFALL; routableNow is the current routing availability. " +
...
function renderMarkdown(
  ...
  const taxonomyRows = (cases: typeof HARD_GOVERNANCE_CASES) =>
    cases.map((entry) => `| ${entry.id} | ${entry.claim} | ${entry.status} |`).join("\n");
...
| --- | --- | --- |
${taxonomyRows(HARD_GOVERNANCE_CASES)}

### Adaptive Runtime

| id | canonical claim | status |
| --- | --- | --- |
${taxonomyRows(ADAPTIVE_RUNTIME_CASES)}

### Authority × Budget

| id | canonical claim | status |
| --- | --- | --- |
${taxonomyRows(AUTHORITY_BUDGET_CASES)}
